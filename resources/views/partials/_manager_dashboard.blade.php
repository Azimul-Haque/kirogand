                    ...
                    @foreach($last5certs as $cert)
                        <li class="item">
                            ...
                            <div class="product-info">
                                <a href="{{ route('dashboard.certificates.draft', $cert->unique_serial) }}" class="product-title font-weight-bold">{{ checkcertificatetype($cert->certificate_type) }} - আইডি: #{{ bangla($cert->unique_serial) }} <i class="fas @if($cert->status == 1) fa-check-circle @else fa-hourglass @endif"></i></a>
